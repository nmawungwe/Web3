...
export default function Home() {
  ...
  const startPresale = async () => {
    ...
    try {
      ...
      const nftContract = new Contract(NFT_CONTRACT_ADDRESS,
                                       NFT_CONTRACT_ABI, 
                                       signer);
  ...
  const checkIfPresaleEnded = async () => {
    try {
      ...
      // This will return a big number because presaleEnded is a uint256
      // This is will timestamp in seconds 
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    //If the user is NOT connect to Rinkeby, tell them to switch to Rinkeby
  ...
  function renderBody() {
    ...
    if (!presaleStarted) {
      // just say that presale hasn't started yet, and come back later 
      return (
        <div>
